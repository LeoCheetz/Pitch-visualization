import ddf.minim.*;
class AudioSource{
  Minim minim; AudioPlayer wav_file; AudioInput microphone;
  ...
  void SetListener(AudioListener listener) {
    ...
    else microphone.addListener (listener);
  }
  ...
  float GetSampleRate() {
    ...
    else return microphone.sampleRate();
  }
  float GetLevel() {
    if (reading_file)
      return wav_file.mix.level();
    else return microphone.mix.level();
  }
}
